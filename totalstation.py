...
class TotalStationImp (object):
    ...
    def _readHeader(self):
        ...
        if QRegExp('\.rw5$', Qt.CaseInsensitive).indexIn(self._file)>-1:
            self._ts = SurvCE(self._file)

        elif QRegExp('\.dat$', Qt.CaseInsensitive).indexIn(self._file)>-1:
            self._ts = Stonex(self._file)
        elif QRegExp('\.gsi$', Qt.CaseInsensitive).indexIn(self._file)>-1:
            self._ts = LeicaGsi(self._file)
        elif QRegExp('\.job$', Qt.CaseInsensitive).indexIn(self._file)>-1:
            self._ts = JobAre(self._file)
        elif QRegExp('\.crd$', Qt.CaseInsensitive).indexIn(self._file)>-1:
            self._ts = Sdr(self._file)

        if self._ts.open() != 0:
            print ("Open error:", self._ts.fname)
        
        
    def _parse(self):
        VermPkt =[]
        while True:
            r = self._ts.parse_next()
            if r is None:
                break
            if len(r) > 0:
                #print (r)
    ...
    def _createVermLayer(self,data):
        name = "Survey"
        offset= 32000000.0 # #only for epsg Germany

        # generate fields with uri
        uri ="Point?field=oid:integer&"
        
        for datpkt  in data[0]:
            uri += "field="+ datpkt 
            wert = str(data[0][datpkt]).replace('.','',1)
            if wert.isdigit()==True:
                if len(str(wert))== len(str(data[0][datpkt])):
                    uri +=  ":int&"
                else:
                    uri +=  ":real&"
            else:
        ...
        uri += "field=art:string&field=strasse:string&field=ortsteil:string"
        ...
        for datpkt in data:
            ...
            fet.setGeometry(QgsGeometry.fromWkt(pt_z))
            lfdnr+=1
            fet['oid']=lfdnr
            for fld in datpkt:
                fet[fld] = datpkt[fld]
        ...
        sldfilename = os.path.normpath(locale_path +'\\Survey.qml')
        #sldfilename = sldfilename.replace('\\','/')

        if os.path.exists(sldfilename):
            # ret = vl.loadSldStyle(sldfilename)
            ret = vl.loadNamedStyle(sldfilename)
            if ret:
                QgsMessageLog.logMessage("Style loaded", 'Messages', Qgis.Info)
            else:
                QgsMessageLog.logMessage("Style NOT loaded", 'Messages', Qgis.Info)
...
class TotalStation(object):
    ...
    def open(self):
        ...
        if self.fp is None:
            try:
                self.fp = open(self.fname, 'r',encoding="utf-8", errors='ignore')
            except IOError:
                self.fp = None
                return -1
        return 0
    ...
    def parse_next(self):
        ...
        while True:
            ...
            if buf is None:
                    return None
            line_code = buf[0]
            self.res['Kennung']=line_code
            if line_code == 'GPS':            # log, lan
                for field in buf[1:]:
                    fcode = field[0:2]
                    if fcode == 'PN':
                        self.res['point_id'] = field[2:].strip()
                        self.lastid=field[2:].strip()
                    elif fcode == 'LA':
                        self.res['la'] = float(field[2:].strip())
                    elif fcode == 'LN':
                        self.res['ln'] = float(field[2:].strip())
                    elif fcode == 'EL':
                        self.res['el'] = float(field[2:].strip())
                    elif fcode == '--':
                ...
                buf = self.get_line()
                line_code = buf[0]
                if line_code == '--GS' or line_code == 'SP':
                ...
                buf = self.get_line() # GT ueberspringen
                buf = self.get_line() 
                line_code = buf[0]
                ...
                if line_code[0:4] == '--HS':
                    ...
                    for field in buf[0:]:
                        fcode = field.split(":")
                        self.res['point_id'] = self.lastid

                        if fcode[0].strip() == '--HSIG':
                            self.res['HSIG'] = float(fcode[1].strip())
                        elif fcode[0].strip() == 'VSIG':
                            self.res['VSIG'] = float(fcode[1].strip())
                        elif fcode[0].strip() == 'STATUS':
                            self.res['STATUS'] = fcode[1].strip()
                        elif fcode[0].strip() == 'SATS':
                            self.res['SATS'] = float(fcode[1].strip())
                        elif fcode[0].strip() == 'PDOP':
                            self.res['PDOP'] = float(fcode[1].strip())
                        elif fcode[0].strip() == 'HDOP':
                            self.res['HDOP'] = float(fcode[1].strip())
                        elif fcode[0].strip() == 'VDOP':
                            self.res['VDOP'] = float(fcode[1].strip())

                    line_code = self.get_line()
                    self.res['DT'] = str(line_code[0][4:14])

                    line_code = self.get_line()
                    self.res['TM'] = str(line_code[0][4:14])
